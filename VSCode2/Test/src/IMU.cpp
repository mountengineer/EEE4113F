...
bool readFifo(LSM6DS3 &myIMU, uint16_t &data_index, float* ax, float* ay, float* az, float* gx, float* gy, float* gz)
{
    ...
    for (uint16_t i = 0; i < fifoCount; i++) {
        // Order is fixed: gx, gy, gz, ax, ay, az
        uint16_t pattern = fifoPattern(myIMU);
        index = data_index + i / AXES;
        temp = myIMU.fifoRead();
        switch (pattern) {
            case 0:
                gx[index] = myIMU.calcGyro(temp);
            break;
            case 1:
                gy[index] = myIMU.calcGyro(temp);    
            break;
            case 2:
                gz[index] = myIMU.calcGyro(temp);
            break;
            case 3:
                ax[index] = myIMU.calcAccel(temp);
            break;
            case 4:
                ay[index] = myIMU.calcAccel(temp);
            break;
            case 5:
                az[index] = myIMU.calcAccel(temp);
            break;
        }
        // uint8_t wordsToSkip = AXES - pattern;
        // for (uint8_t i = 0; i < wordsToSkip; i++) {
        //     myIMU.fifoRead();   // discard
        // }
        // fifoCount -= wordsToSkip;
    }
    data_index += fifoCount / AXES;
    return data_index >= UNLOADS_PER_DEC * FIFO_THRESHOLD / AXES;
}
*/
uint16_t readFifo(LSM6DS3 &myIMU, uint16_t &data_index) {
    uint16_t fifoStatus = myIMU.fifoGetStatus();
    uint16_t fifoCount  = fifoStatus & 0x0FFF;
    uint16_t numSamples = fifoCount / AXES;
    uint16_t remainder  = fifoCount % AXES;

    uint16_t maxSamples = (UNLOADS_PER_DEC * FIFO_THRESHOLD / AXES) - data_index;
    if (numSamples > maxSamples) numSamples = maxSamples;

    // Start DMA transfer — returns immediately
    startDMARead(numSamples * AXES);
    return numSamples;
}
